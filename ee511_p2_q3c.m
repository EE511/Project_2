...
%%checks the Goodness of Fit test to validate if the samples fit the Dist 1,2..9 or not
%%Here NULL hypothesis is that the sample fits the distribution 1,2...,9,10
...
function [ ] = goodness_of_fit(M)
  ...
  for expt = 1:length(trails)
      ...
      disp('Summary for Discrete uniform distribution 1,2,...10 for');          
      for bin_index=1:length(bin_count)
          ...
          edges = linspace(1,M,(bin_count(bin_index)+1));
